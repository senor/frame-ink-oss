...
export function SegmentedControl({ options, value, onChange, label, className = '' }: SegmentedControlProps) {
    return (
        <div className={`space-y-4 ${className} mono-font relative`}>
            {label && <label className="text-[11px] block font-black uppercase tracking-widest text-ink opacity-70">{label}</label>}
                ...
                {options.map((opt, i) => {
                    ...
                    return (
                        <button
                            ...
                            type="button"
                            className={`
                                flex-1 font-black text-[13px] uppercase transition-all duration-200 cursor-pointer h-12 relative z-10
                                ${isActive
